...
/**
 * Public Privacy Policy page for houseofjainz.com/privacypolicy.
 * Fetches and displays the latest privacy document; no login required.
 */
export default function PublicPrivacyView() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) => {
        if (!cancelled) {
          setError(err.response?.status === 404 ? 'Privacy policy not found' : 'Failed to load');
        }
      })
      .finally(() => {
  ...
  if (error || !document) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.errorText}>{error || 'Privacy policy not available'}</Text>
