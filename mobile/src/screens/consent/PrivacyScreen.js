...
export default function PrivacyScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    if (consents && consents.privacy) {
      setAlreadyAccepted(hasConsented('privacy'));
    }
  }, [consents]);
  ...
  const loadDocument = async () => {
    try {
      setLoading(true);
      const result = await getConsentDocument('privacy');
      if (result.success) {
        ...
        if (result.notFound) {
          Alert.alert(
            ...
            'Privacy Policy document is not available. Please contact support or try again later.',
            [{ text: 'OK', onPress: () => navigation.goBack() }]
          );
        } else {
          Alert.alert('Error', result.error || 'Failed to load Privacy Policy');
        }
      }
    } catch (error) {
  ...
  const handleAccept = async () => {
    ...
    setAccepting(true);
    try {
      const result = await grantConsent('privacy', document.version);
      if (result.success) {
        if (onAccept) {
          onAccept();
        } else {
        ...
        Alert.alert('Error', result.error || 'Failed to accept privacy policy');
      }
    } catch (error) {
